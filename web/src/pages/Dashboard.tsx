...
export default function Dashboard() {
  ...
  return (
    // Alterado para flex-col no mobile, xl:flex-row no desktop grande
    <div className="flex flex-col xl:flex-row gap-8 items-start max-w-[1600px] mx-auto min-h-screen">
          ...
          <>
            ...
            {!data?.cicloId ? (
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col overflow-hidden">
                {/* Header Tabela - Escondido no mobile se quiser economizar espaço, mas mantive ajustado */}
                ...
                <div className="divide-y divide-gray-50">
                  {data.itens.map((item, idx) => (
                    <div key={idx} className="grid grid-cols-12 gap-2 sm:gap-4 px-4 sm:px-6 py-3 sm:py-2.5 items-center hover:bg-blue-50/30 transition-colors group">
                      ...
                      {/* Barras de Progresso - Maior altura no mobile */}
                      <div className="col-span-4 sm:col-span-4 flex flex-col justify-center gap-1.5 px-1 sm:px-2">
                        <div className="relative w-full h-3 sm:h-2.5 bg-gray-100 rounded-full overflow-hidden" title={`Horas: ${item.percentualHoras.toFixed(0)}%`}>
                        ...
                        {item.metaQuestoes > 0 && (
                          <div className="relative w-full h-3 sm:h-2.5 bg-gray-100 rounded-full overflow-hidden" title={`Questões: ${item.percentualQuestoes.toFixed(0)}%`}>
      ...
      {/* Coluna Direita: Registro Rápido (Escondida no Mobile, pois vai pro Modal/FAB) */}
